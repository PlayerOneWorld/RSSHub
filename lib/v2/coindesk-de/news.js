...
module.exports = async (ctx) => {
    ...
    const filter_time = Number(ctx.query.filter_time) || 0;
    const comTime = ((Date.now() / 1000) >> 0) - filter_time;
    const filterTimeList = filter_time > 0 ? newsList.filter((item) => (new Date(item.pubDate).getTime() / 1000) >> 0 >= comTime) : newsList;
